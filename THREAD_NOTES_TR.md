...
5. Araç yalnızca `GET /r/<room>/export` çağrısı yapar; public DID ile lokal filtreler, export içindeki `room|nonce|text` Ed25519 imzasını yerelde tekrar doğrular ve JSON/Markdown kanıt raporu üretir.
...
Güvenlik ilkesi net: seed/private key asla uygulamaya girmez, uygulama yalnızca read-only `GET /r/<room>/export` yapar. Raporun kaynak export SHA-256'sı da yer alıyor; bu, raporu bir ödül/airdrop kanıtına dönüştürmez.
